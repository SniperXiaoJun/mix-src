...
{
    CGeneralMsgClass msgRecv(byteArray.constData(),byteArray.count());
			...
			case USR_LOGIN:
				{
					...
					if(!bOnLine)
					{
                        CBaseDataField field = msgRecv.GetField(i,j);
...
int YY_CHAT::SendMSG_LOGIN(QString strIP, QString strMSG)
{
    ...
    CBaseDataField fieldLogin(USR_LOGIN);

    int logValue = USR_LOGIN;

    fieldLogin.SetValue((const char *)&logValue, 4);
	msgLogin.AddField(fieldLogin);

    CBaseDataField fieldName(USR_NAME);

    fieldName.SetValue(m_sYY_CHAT_USR_Self.strName.toAscii().data(), strlen((m_sYY_CHAT_USR_Self.strName.toAscii().data())));
    msgLogin.AddField(fieldName);

    CBaseDataField fieldComment(USR_COMMENT);
    fieldComment.SetValue(m_sYY_CHAT_USR_Self.strNote.toAscii().data(), strlen((m_sYY_CHAT_USR_Self.strNote.toAscii().data())));
...
int YY_CHAT::SendMSG_LOGOUT(QString strIP, QString strMSG)
{
    ...
    CBaseDataField fieldLogOut(USR_LOGOUT);

    int logoutValue = USR_LOGOUT;

    fieldLogOut.SetValue((const char *)&logoutValue, 4);
...
int YY_CHAT::SendMSG_SendMSG(QString strIP, QString strMSG)
{
    ...
    CBaseDataField fieldMSG(USR_MSG);

    fieldMSG.SetValue(strMSG.toAscii().data(), strlen(strMSG.toAscii().data()));
