...
COMMON_API unsigned int __stdcall WTF_SM2SignDigestForHengBao(SK_CERT_DESC_PROPERTY * pCertProperty,unsigned int ulPINType , CallBackCfcaGetEncryptPIN GetEncryptPIN, void * pArgs/*NULL is able*/, unsigned int *puiRetryCount, BYTE *pbData, unsigned int uiDataLen, PECCSIGNATUREBLOB pSignature)
{
	...
	ulRet = WTF_SM2SignInitializeVerifyPINByCertProperty(pCertProperty,ulPINType,GetEncryptPIN,&args,puiRetryCount);
	if (0 != ulRet)
	{
		goto err;
	}
...
unsigned int __stdcall WTF_SM2SignInitializeVerifyPINByCertProperty(SK_CERT_DESC_PROPERTY * pCertProperty,unsigned int ulPINType , CallBackCfcaGetEncryptPIN GetEncryptPIN, OPST_HANDLE_ARGS * args, unsigned int *puiRetryCount)
{
	HINSTANCE ghInst = NULL;
...
unsigned int __stdcall WTF_SM2SignFinalize(OPST_HANDLE_ARGS * args)
{
	HINSTANCE ghInst = NULL;
	unsigned int ulRet = 0;
...
{
	return WTF_EnumCertInternalBySKF(pCertProperty->szSKFName,pvCertsValue,puiCertsLen,ulKeyFlag,ulSignFlag,ulVerifyFlag, ulFilterFlag);
}
